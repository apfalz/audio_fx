...
def add_delay(data, seed, fs=44100):
    np.random.seed(seed)
    output = []
    for d in data:
        offset = int(np.random.random()*fs)
        echo   = (np.roll(d, offset) * 0.5 ) + np.array(d)
        output.append(echo / np.amax(echo))
    return output
...
if __name__ == '__main__':
    ...
    onsets   = lib.onset.onset_detect(y=data, sr=fs, hop_length=512, units='samples')

    peaks, chunks = get_chunks(data, onsets, min_length=10000)
    procs = []
    for p in range(3):
        procs.append(Process(target=create_track, args=(chunks, onsets, p)))
